    ...
    for filename in file_list:
        ...
        if filename.endswith(".pkl") and filename.startswith(names_start_with):
            ...
            result = {
                ...
                "test_paulis_file": f'../experiments/results_google/test_paulihedral_' + new_filename
            }
            print(result)
            results.append(result)
            if save_output == True:
                # Save test_paulis to a separate JSON file
                with open(f'../experiments/results_google/test_paulihedral_' + new_filename, 'w') as paulis_file:
...
def run_benchmarks(config):
    ...
    for name in names_start_with[config]:
        print(f"Running benchmarks in folder: ../experiments/paulihedral_partial_results_google/")
        run_experiment_folder(folder_path="../experiments/paulihedral_partial_results_google/", save_output=True, names_start_with=name)
