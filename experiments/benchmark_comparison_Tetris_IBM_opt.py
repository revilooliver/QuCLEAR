    ...
    for filename in file_list:
        ...
        if filename.endswith(".pkl") and filename.startswith(names_start_with):
            ...
            result = {
                ...
                "test_paulis_file": f'../experiments/results_ibm/test_tetris_' + new_filename
            }
            print(result)
            results.append(result)
            if save_output == True:
                # Save test_paulis to a separate JSON file
                with open(f'../experiments/results_ibm/test_tetris_' + new_filename, 'w') as paulis_file:
...
def run_benchmarks(config):
    ...
    for name in names_start_with[config]:
        print(f"Running benchmarks in folder: ../experiments/tetris_partial_results_ibm/")
        run_experiment_folder(folder_path="../experiments/tetris_partial_results_ibm/", save_output=True, names_start_with=name)
